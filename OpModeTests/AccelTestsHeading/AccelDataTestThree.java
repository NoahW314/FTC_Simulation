...
@TeleOp(name="Three Step", group="testing")
@Disabled
public class AccelDataTestThree extends LinearOpModeSim {

	//the possible states of the program
	private enum states{drive1, drive2, drive3, done};
    ...
    private double driveTime1 = 1.75;
    ...
    //the angle the robot will drive at in the third step
    private int driveAngle3 = 0;
    //the time the robot will drive for in the third step
    private double driveTime3 = 1.25;
    ...
    //the data logger
    private DataLoggerSim IMUlogger = new DataLoggerSim("Three Step Test "+testNum);
	...
	public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
        	switch(state) {
        	//drive for a set time through three different steps, logging the accelerometer data throughout, then stop
        		case drive1:
        		...
        		case drive2:
        			...
        			break;
        		case drive3:
        			telemetry.addData("State: ", "Driving 3");
        			//drive
        			robotDrive.driveAtAngle(driveAngle3, 1);
        			robotDrive.drive(speed);
        			if(time.seconds() > driveTime3) {
        				//stop everything
        				state = states.done;
        				robotDrive.drive(0);
