...
public class BNO055TestMulti extends BNO055IMU {
    ...
    // Start integrating acceleration to determine position and velocity by polling for acceleration every while
    {
        ...
        synchronized (this.startStopLock)
        {
            ...
            for(int i = 0; i < msPollInterval.length; i++) {
            	accelerationAlgorithms[i] = ((FileLoggingAccelerationIntegratorHeading)this.accelerationAlgorithm).clone(i);
            }
            ...
            public void run() {
                ...
                try
                {
                    ...
                    while (!isStopRequested())
                    {
                        ...
                        final Acceleration linearAcceleration = BNO055TestMulti.this.getLinearAcceleration();
                        final double heading = BNO055TestMulti.this.getAngularOrientation(
                        ...
                        {
                            ((FileLoggingAccelerationIntegratorHeading)accelerationAlgorithms[i]).update(linearAcceleration, heading);
                        }
